...
export default defineComponent({
  ...
  data() {
    ...
    const dataset = {
      ...
      rowHeights: {}, // Cache for calculated row heights
      autoRowHeight: false, // Enable/disable auto row height
      minRowHeight: 24, // Minimum row height
      maxRowHeight: 200, // Maximum row height
      rowHeightCache: new Map(), // Cache for computed row heights
      resizeObserver: null, // ResizeObserver for dynamic content
    };
    return dataset;
  },
  computed: {
  ...
  watch: {
    ...
    autoRowHeight(newVal) {
      if (newVal) {
        ...
        if (this.recordBody) {
          Array.from(this.recordBody.children).forEach((row) => {
            row.style.height = '';
  ...
  methods: {
     ...
     */
    calculateRowHeights() {
      if (!this.autoRowHeight || !this.recordBody) return;

      this.rowHeights = {};
      this.rowHeightCache.clear();

      // Calculate heights for visible rows
      const visibleRows = this.pagingTable;
      visibleRows.forEach((record, rowPos) => {
        const height = this.calculateSingleRowHeight(record, rowPos);
        if (height > this.minRowHeight) {
          this.rowHeights[rowPos] = height;
        }
      });

      // Apply heights to DOM
      this.applyRowHeights();
    },

    calculateSingleRowHeight(record, rowPos) {
      const cacheKey = this.getRowCacheKey(record, rowPos);
      if (this.rowHeightCache.has(cacheKey)) {
        return this.rowHeightCache.get(cacheKey);
      }

      let maxHeight = this.minRowHeight;

      if (this.recordBody && this.recordBody.children[rowPos]) {
        const rowElement = this.recordBody.children[rowPos];
        const cells = Array.from(rowElement.children).slice(1); // Skip first-col

        cells.forEach((cell, cellIndex) => {
          if (cellIndex >= this.fields.length) return;

          const field = this.fields[cellIndex];
          if (field && !field.invisible) {
            const cellHeight = this.calculateCellHeight(cell, record, field);
      ...
      const clampedHeight = Math.min(
        Math.max(maxHeight, this.minRowHeight),
        this.maxRowHeight
      );

      this.rowHeightCache.set(cacheKey, clampedHeight);
      return clampedHeight;
    },

    calculateCellHeight(cell, record, field) {
      if (!cell) return this.minRowHeight;

      // Get the actual content height by creating a temporary measurement element
      const content = field.toText(record[field.name], record, field, 0);

      if (!content || content.trim() === '') return this.minRowHeight;
      ...
      return Math.max(height + 8, this.minRowHeight); // Add padding
    },

    getRowCacheKey(record, rowPos) {
      const cellContents = this.fields
      ...
      return `${record.$id}:${cellContents}`;
    },

    applyRowHeights() {
      if (!this.recordBody) return;

      Array.from(this.recordBody.children).forEach((row, index) => {
        const rowPos = this.pageTop + index;
        const record = this.table[rowPos];

        if (
          record &&
          this.rowHeights[index] &&
          this.rowHeights[index] > this.minRowHeight
        ) {
          const height = this.rowHeights[index];
          row.style.height = height + 'px';

          // Adjust all cells in the row
          Array.from(row.children).forEach((cell) => {
            if (cell.tagName === 'TD') {
      ...
      // Update vertical scrollbar calculations
      this.updateVerticalScrollForRowHeights();
    },

    updateVerticalScrollForRowHeights() {
      // Recalculate total table height considering dynamic row heights
      let totalHeight = 0;

      // Add header heights
      if (this.labelTr) totalHeight += this.labelTr.offsetHeight;
      if (this.filterRow && this.filterTr)
        totalHeight += this.filterTr.offsetHeight;

      // Calculate visible rows height
      if (this.recordBody) {
        Array.from(this.recordBody.children).forEach((row, index) => {
          const height = this.rowHeights[index] || this.minRowHeight;
          totalHeight += height;
        });
      }

      // Add footer and summary heights
      if (!this.noFooter && this.footer)
        totalHeight += this.footer.offsetHeight;
      if (this.summaryRow) totalHeight += 27; // Summary row height

      // Update container height if needed
      if (this.tableContent && totalHeight > 0) {
        const containerHeight = this.tableContent.offsetHeight;
        if (totalHeight <= containerHeight) {
          this.tableContent.style.overflowY = 'hidden';
        } else {
          this.tableContent.style.overflowY = 'auto';
        }
      }

      // Recalculate vertical scroll
      this.calVScroll();
    },

    updateRowHeight(rowPos, record) {
      ...
      if (height > this.minRowHeight) {
        ...
        delete this.rowHeights[rowPos];
      }

      this.applyRowHeights();
    },

    /* *** End Row Auto Size Feature ****************************************************************************************
     */
    calTable() {
      this.textTip = '';
            ...
            for (let i = 0; i < filterColumnList.length; i++) {
              ...
              switch (filter[k].type) {
                ...
                case 6:
                  ...
                  break;
                case 7:
                  if (!content[k].startsWith(filter[k].modelValue))
                    return false;
                  break;
                case 8:
    ...
    */
    ...
    mouseDown(e) {
      if (
        ...
        if (this.currentField.listByClick)
          return this.calAutocompleteList(true);
    ...
    async updateCell(row, field, newVal, isUndo) {
      ...
      setTimeout(() => {
        ...
        // Update row height if auto row height is enabled
        if (this.autoRowHeight) {
          this.updateRowHeight(rowPos, row);
        }
